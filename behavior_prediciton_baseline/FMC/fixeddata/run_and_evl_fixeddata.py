...
def create_model(dataset, k_embedding=64):
    config = DATASET_CONFIG[dataset]
    model = FMC.FPMC(
        actionNum=config['action_num'],
        deviceNum=config['device_num'],
...
def train_model(model, train_loader, val_loader, args, save_dir):
    ...
    for epoch in range(args.epochs):
        ...
        for batch in train_bar:
            days, times, devices, actions, masks, targets = batch
            days = days.to(device)
            ...
            optimizer.zero_grad()
            outputs = model(days, times, devices, actions, masks)
...
def run_experiment(dataset, environment, args):
    ...
    model = create_model(dataset, args.k_embedding)
...
def main():
    parser = argparse.ArgumentParser(description='FMC Fixed Length Action-Only Training')
    ...
    parser.add_argument('--k_embedding', type=int, default=64, help='Embedding dimension')
