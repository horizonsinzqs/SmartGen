...
def Transsas_baseline(dataset, ori_env):
    with open(f'baseline_data/{dataset}/{ori_env}/trn.pkl', 'rb') as file2:
        sequence = pickle.load(file2)
    actions = sequence
...
def Transsas_test_baseline(dataset, ori_env):
    with open(f'baseline_data/{dataset}/{ori_env}/test.pkl', 'rb') as file2:
        sequence = pickle.load(file2)
    actions = sequence

    with open(f'data/{dataset}_{ori_env}_test.txt', 'w') as file:
        pass
    ...
    for t in range(len(actions)):
        ...
        with open(f'data/{dataset}_{ori_env}_test.txt', 'a') as file:
