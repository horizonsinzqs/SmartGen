...
from transsas import Transsas_test_baseline, Transsas_baseline
from utils import *
...
def SASRec_behavior_prediction_baseline(nation, ori_env, need):
    if need == 'train':
        ...
        Transsas_test_baseline(nation, ori_env)
        parser = argparse.ArgumentParser()
        parser.add_argument('--dataset', default=f'{nation}_{ori_env}_test')
        parser.add_argument('--train_dir', default='default')
        parser.add_argument('--batch_size', default=128, type=int)
        parser.add_argument('--lr', default=0.001, type=float)
        parser.add_argument('--maxlen', default=200, type=int)
...
if __name__ == '__main__':
    ...
    for nation in datasets:
        for ori_env in environments:
            # SASRec_behavior_prediction_baseline(nation, ori_env, 'train')
            test_metrics = SASRec_behavior_prediction_baseline(nation, ori_env, 'test')
            result = {
                'dataset': nation,
                'environment': ori_env,
                'model': 'SASRec_fixed',
