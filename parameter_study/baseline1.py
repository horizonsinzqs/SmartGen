...
def Anomaly_detection(dataset, new_env, thres, method, model, study, per):
    model_name = f"check_model/best_{study}_{dataset}_{new_env}_generation_{method}_th={thres}_{model}_model.pth"
...
def Anomaly_detection_splitEP(dataset, new_env, thres, method, model, study, per, sp1, sp2):
    model_name = f"check_model/best_{study}_{dataset}_{new_env}_generation_{method}_th={thres}_{model}_model_{sp1}_{sp2}.pth"
    vocab_size = vocab_dic[dataset]
    epochs = 15
    seq_len = 10

    data_file = f'study_data/{study}/{dataset}_{new_env}_generation_{method}_th={thres}_{model}_seq_filter_true_{sp1}_{sp2}.pkl'
